...
product = Blueprint("products", __name__, url_prefix="/product")


@product.route("/", methods=["GET"])
def product_index():
    products = Product.query.options(joinedload("user")).all()
...
def product_create(user):
    ...
    user.product.append(new_product)
    db.session.commit()
...
def product_update(user, id):
    product_fields = product_schema.load(request.json)
    product = Product.query.filter_by(id=id, user_id=user.id)
